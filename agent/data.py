...
It just reads the JSON files that were created by running:
    ...
    npm run export:data   (from the project root)
...
we just made Python-readable copies of them.
  ...
  data/constraints.json   — 27 travel warnings (public holidays, weather, …)
...
# These variables are loaded once when Python first imports this file.
...
CUISINE: list[dict] = _load("cuisine.json")           # type: ignore[assignment]
...
CITY_CONNECTIONS: list[dict] = _load("city_connections.json")  # type: ignore[assignment]
TRANSPORT_HUBS: list[dict] = _load("transport_hubs.json")  # type: ignore[assignment]
